...
<?php 
  ...
  {
    extract($_POST);
    $q1=$dbb->query('SELECT mot_de_passe FROM Personnel WHERE mot_de_passe="'.$a_mdp.'"') or die("Erreur suppression : " . $link->error);
   $d=$q1->fetch();
   ...
   if($d==true)
   {
    if($n_mdp==$cn_mdp)
    {
    $q2=$dbb->query('UPDATE Personnel SET mot_de_passe="'.$n_mdp.'" WHERE nom="'.$nom .'" AND prenom="'.$prenom.'"');
    }
